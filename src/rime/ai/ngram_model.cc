...
std::string CtxKey(const std::vector<int32_t>& ids) {
  ...
  for (int32_t id : ids) s.append(reinterpret_cast<const char*>(&id), sizeof(id));
  return s;
}

void WriteU32(std::ostream& os, uint32_t v) { os.write(reinterpret_cast<const char*>(&v), 4); }
void WriteI64(std::ostream& os, int64_t v) { os.write(reinterpret_cast<const char*>(&v), 8); }
void WriteF64(std::ostream& os, double v) { os.write(reinterpret_cast<const char*>(&v), 8); }
...
void WriteI64s(std::ostream& os, const std::vector<int64_t>& v) {
  ...
  for (int64_t x : v) WriteI64(os, x);
}

std::vector<int64_t> ReadI64s(std::istream& is) {
  const uint32_t n = ReadU32(is);
  std::vector<int64_t> v(n);
  for (uint32_t i = 0; i < n; ++i) v[i] = ReadI64(is);
  return v;
}

void CheckStream(std::istream& is, const char* what) {
  if (!is) throw std::runtime_error(std::string("corrupt or truncated model: ") + what);
}
...
void Vocab::Load(std::istream& is) {
  word2id_.clear();
  id2word_.clear();
  Add("<unk>");  // re-register id 0 so the file's own "<unk>" dedups instead of shifting ids
  const uint32_t n = ReadU32(is);
...
void NGramModel::Train(const std::vector<std::vector<std::string>>& lines, int32_t order,
                       int32_t min_count, double discount) {
  ...
  for (int32_t n = 2; n <= order_; ++n) {
    for (const auto& l : ids) {
      if (static_cast<int32_t>(l.size()) < n) continue;
  ...
  for (const auto& kv : follow_)
    if (kv.first.size() == sizeof(int32_t))  // order-2 contexts only
      for (const auto& fw : kv.second) ++continuation_[static_cast<size_t>(fw.first)];
  total_continuation_ = 0;
  for (int64_t c : continuation_) total_continuation_ += c;

  RebuildDerived();
}

double NGramModel::P1(int32_t word) const {
  if (total_continuation_ == 0) return 0.0;
...
double NGramModel::P(int32_t word, const std::vector<int32_t>& context) const {
  double prob = P1(word);
  const int32_t max_len = std::min<int32_t>(order_ - 1, static_cast<int32_t>(context.size()));
  for (int32_t len = 1; len <= max_len; ++len) {
    std::vector<int32_t> ctx(context.end() - len, context.end());
    auto it = follow_.find(CtxKey(ctx));
    if (it == follow_.end()) continue;  // unseen context: back off, prob unchanged
    const auto& f = it->second;
    int64_t c_h = 0;
    for (const auto& kv : f) c_h += kv.second;
    const auto wi = f.find(word);
    const double c_hw = wi == f.end() ? 0.0 : static_cast<double>(wi->second);
    const double alpha = discount_ * static_cast<double>(f.size()) / static_cast<double>(c_h);
    prob = std::max(c_hw - discount_, 0.0) / static_cast<double>(c_h) + alpha * prob;
  }
  return prob;
}

std::vector<NGramModel::Candidate> NGramModel::TopK(const std::vector<std::string>& context,
                                                    int32_t k) const {
  k = std::max(1, k);
  ...
  for (auto it = context.rbegin();
       it != context.rend() && ids.size() < static_cast<size_t>(order_ - 1); ++it) {
    const int32_t id = vocab_.Id(*it);
    if (id == Vocab::kUnknown) break;
    ids.push_back(id);
  }
  ...
  for (int32_t len = 1; len <= static_cast<int32_t>(ids.size()); ++len) {
    auto it = follow_.find(CtxKey(std::vector<int32_t>(ids.end() - len, ids.end())));
    if (it == follow_.end()) continue;
    for (const auto& kv : it->second) pool.push_back(kv.first);
  }
  for (int32_t w : top_continuation_) {
    if (static_cast<int32_t>(pool.size()) >= k) break;
    pool.push_back(w);
  }
  ...
  for (int32_t w : pool) out.push_back({w, P(w, ids)});
  std::sort(out.begin(), out.end(),
            [](const Candidate& a, const Candidate& b) { return a.prob > b.prob; });
  if (static_cast<int32_t>(out.size()) > k) out.resize(static_cast<size_t>(k));
  return out;
}
...
void NGramModel::RebuildDerived() {
  ...
  for (int32_t w = 0; w < static_cast<int32_t>(vocab_.size()); ++w)
    if (continuation_[static_cast<size_t>(w)] > 0) top_continuation_.push_back(w);
  std::sort(top_continuation_.begin(), top_continuation_.end(), [this](int32_t a, int32_t b) {
    return P1(a) > P1(b);
  });
}
...
void NGramModel::Load(std::istream& is) {
  char magic[4] = {};
  is.read(magic, 4);
  if (std::memcmp(magic, "AILM", 4) != 0) throw std::runtime_error("not an AILM model file");
